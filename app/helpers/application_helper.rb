module ApplicationHelper
  ...
  def project_icon_path project, size
    if project.icon.nil?
      @img_path = "/images/generic_project_icon.png"
    else
  ...
  def blog_body blog
    ...
    @blog_body = blog.body.gsub( /@\w+/) do |mention|
      ...
      link_to mention, "#{root_url}#{@user_login}"
    end
    ...
    @blog_body = @blog_body.gsub( /\*\w+/) do |mention|
      ...
      (@mentioned_project and @project_symbol.size == 5) ? link_to(mention.upcase, "#{root_url}projects/#{@project_symbol.upcase}", :title => "#{h @mentioned_project.title}") : mention
    end
